...
namespace AppointmentService.FunctionalTests.Tests
{
    public class UpdateAppointmentTests : BaseFunctionalTest
    {
        ...
        public async Task UpdateAppointment_ShouldReturnOk_WhenValidRequest()
        {
            // Arrange
            SetAuthHeader();
            var create_request = new CreateAppointmentRequest(1, DateTime.UtcNow.AddDays(1), AppointmentType.Vaccination, "PATIENT1", "Patient 1", DateTime.UtcNow.AddDays(-1), "patient@example.com", "84123456789", "adada", null, 1, 1, "");
