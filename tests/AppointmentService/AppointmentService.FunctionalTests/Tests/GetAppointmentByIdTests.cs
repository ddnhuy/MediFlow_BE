...
namespace AppointmentService.FunctionalTests.Tests
{
    public class GetAppointmentByIdTests : BaseFunctionalTest
    {
        ...
        public async Task GetAppointmentById_ShouldReturnOk_WhenUserIsAuthenticated()
        {
            // Arrange
            SetAuthHeader();
            var create_request = new CreateAppointmentRequest(1, DateTime.UtcNow.AddDays(1), AppointmentType.Vaccination, "PATIENT1", "Patient 1", DateTime.UtcNow.AddDays(-1), "patient@example.com", "84123456789", "adas", null, 1, 1, "");
